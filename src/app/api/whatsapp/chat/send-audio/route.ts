...
import { esquemaWhatsappChatSendAudio } from "@/lib/validacoes";
import {
  buscarPdvDoLead,
...
export async function POST(request: NextRequest) {
  return withSessao(request, async ({ sessao }): Promise<NextResponse> => {
    ...
    const validacao = validateBody(esquemaWhatsappChatSendAudio, parseResult.data);
    if (!validacao.ok) return validacao.response;

    const { leadId, audioBase64, clientTempId } = validacao.data;
    ...
    try {
      const sendResponse = await enviarAudioWhatsapp(
      ...
      return serverError(erro instanceof Error ? erro.message : "Erro ao enviar audio.");
    }
  });
}
