...
import { esquemaWhatsappChatSendMedia } from "@/lib/validacoes";
import {
  buscarPdvDoLead,
...
export async function POST(request: NextRequest) {
  return withSessao(request, async ({ sessao }): Promise<NextResponse> => {
    ...
    const validacao = validateBody(esquemaWhatsappChatSendMedia, parseResult.data);
    if (!validacao.ok) return validacao.response;

    const { leadId, mediaBase64, mimeType, fileName, caption, clientTempId } = validacao.data;
    ...
    if (!instancia) {
      ...
      const podeConfigurar =
        ...
        (sessao.perfil === "GERENTE" && sessao.id_pdv === pdv?.id);
      ...
      return NextResponse.json(
        {
          ...
          rotaConfiguracao:
            podeConfigurar && pdv
              ? `/equipe?id_pdv=${pdv.id}&editar_pdv=${pdv.id}`
              : null,
        },
        { status: 409 },
      );
    ...
    try {
      const sendResponse = await enviarMidiaWhatsapp(
      ...
      return ok({
        message: mapearMensagemDbParaCanonica(persisted),
        clientTempId,
      });
    } catch (erro) {
      return serverError(erro instanceof Error ? erro.message : "Erro ao enviar midia.");
    }
  });
}
