...
export async function POST(request: NextRequest) {
  ...
  if (!nome || !telefone || !id_estagio || !id_funcionario || valor_consorcio <= 0) {
    return NextResponse.json({ erro: "Dados do lead invalidos." }, { status: 400 });
  }

  const [estagio, funcionario] = await Promise.all([
    prisma.estagioFunil.findFirst({
      where: { id: id_estagio, id_empresa: auth.sessao.id_empresa },
    }),
    prisma.funcionario.findFirst({
      where: { id: id_funcionario, id_empresa: auth.sessao.id_empresa, ativo: true },
    }),
  ]);
