...
type PdvManagementPanelProps = {
  ...
  onSelecionarPdv?: (pdvId: string) => void;
};

export function PdvManagementPanel({ vm, drawerNovoPdvAberto, setDrawerNovoPdvAberto, onSelecionarPdv }: PdvManagementPanelProps) {
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-foreground-muted">Clique em um PDV abaixo para gerenciar a equipe e ver detalhes da operação.</p>
        </div>
                  ...
                  <>
                    ...
                    >
                      ...
                      <button
                        ...
                        onClick={(e) => {
                          e.stopPropagation();
                          handleCardClick(pdv.id);
                        }}
                        className="mt-2 flex w-full items-center justify-center gap-1.5 rounded-xl border border-border/60 bg-muted/40 px-3 py-1.5 text-xs font-semibold text-foreground-muted transition-all hover:border-info/30 hover:bg-info/10 hover:text-info group-hover:border-info/20"
                      >
                        Gerenciar equipe
