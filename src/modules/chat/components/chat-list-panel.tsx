...
export function ChatListPanel({
  ...
  return (
    <div className="flex h-full min-h-0 flex-col bg-background-surface">
      ...
      <div className="flex-1 overflow-y-auto scrollbar-thin scrollbar-thumb-border scrollbar-track-transparent hover:scrollbar-thumb-foreground-muted/40">
        {process.env.NODE_ENV === "development" && (
          <div className="border-b border-dashed border-border px-4 py-2 text-[11px] text-foreground-muted">
        ...
        {!carregando && conversas.length === 0 && (
          <div className="flex flex-col items-center justify-center gap-2 px-4 py-12 text-center">
            <MessageSquareOff className="h-8 w-8 text-foreground-muted/60" />
        ...
        {erro && (
          <div className="px-4 py-3">
            <p className="rounded-lg border border-destructive/20 bg-destructive/10 px-3 py-2 text-xs text-destructive">{erro}</p>
          </div>
        )}
        ...
        {temMais && (
          <div className="p-3">
            <button
              ...
              className="w-full rounded-lg border border-border py-2 text-xs text-foreground-muted hover:bg-muted disabled:opacity-50"
            >
              {carregandoMais ? (
