...
- [x] 1.1 Adicionar model `TransferenciaLead` ao `prisma/schema.prisma` com campos `id`, `id_lead`, `id_funcionario_origem`, `id_funcionario_destino`, `status`, `criado_em`, `respondido_em` e relações FK com `Lead` e `Funcionario`
- [x] 1.2 Adicionar `@@unique([id_lead, status])` com condição parcial `WHERE status = 'PENDENTE'` via custom migration (garantir no máximo 1 convite pendente por lead)
- [x] 1.3 Adicionar índices em `id_funcionario_origem` e `id_funcionario_destino`
- [x] 1.4 Gerar migration com `prisma migrate dev` e validar que o banco reflete o schema

## 2. API — Nova rota de transferência

- [x] 2.1 Criar `src/app/api/leads/[id]/transferencia/route.ts` com handlers `POST`, `PATCH` e `DELETE`
- [x] 2.2 Implementar `POST`: validar que COLABORADOR logado é dono do lead; validar `id_funcionario_destino` (COLABORADOR ativo, mesmo PDV, mesma empresa); validar que não existe `TransferenciaLead` PENDENTE para o lead; criar registro com status `PENDENTE`; retornar `201`
- [x] 2.3 Implementar `PATCH`: validar que COLABORADOR logado é o destinatário do convite; validar que status atual é `PENDENTE`; se `acao = "ACEITAR"`, usar `prisma.$transaction` para atualizar `lead.id_funcionario` + `TransferenciaLead.status = "ACEITA"` + `respondido_em`; se `acao = "RECUSAR"`, atualizar status para `RECUSADA` + `respondido_em`
- [x] 2.4 Implementar `DELETE`: validar que COLABORADOR logado é o remetente do convite; validar que status atual é `PENDENTE`; atualizar status para `RECUSADA` + `respondido_em` (soft cancel)
- [x] 2.5 Adicionar esquema Zod `esquemaTransferirLead` em `src/lib/validacoes.ts` para validar payload (POST: `{ id_funcionario_destino }`, PATCH: `{ acao: "ACEITAR" | "RECUSAR" }`)
- [x] 2.6 Adicionar helper `obterTransferenciaPendenteLead(leadId)` para queries de busca de transferência por lead

## 3. API — Modificação do GET /api/leads

- [x] 3.1 Modificar `GET /api/leads` (`src/app/api/leads/route.ts`) para incluir dados de `TransferenciaLead` PENDENTE no response de cada lead (campo opcional `transferencia_pendente` com origem, destino, status, criado_em)
- [x] 3.2 Estender `whereLeadsPorPerfil` em `src/lib/permissoes.ts` para COLABORADOR: além dos próprios leads (`id_funcionario = self`), incluir leads com `TransferenciaLead` PENDENTE onde `id_funcionario_destino = self` (para popular coluna de transferências recebidas)
- [x] 3.3 Garantir que leads com transferência pendente recebida (destinatário) são retornados com os mesmos includes (funcionario, pdv, gestores, parcelas) que leads próprios

## 4. Frontend — Tipos e dados

- [x] 4.1 Adicionar type `TransferenciaPendente` em `src/modules/kanban/types.ts` com campos: `id`, `status`, `origem: { id, nome }`, `destino: { id, nome }`, `criado_em`
- [x] 4.2 Adicionar campo opcional `transferencia_pendente` no type `Lead`
- [x] 4.3 Adicionar funções de API client em `src/lib/api/kanban.ts`: `criarTransferencia(idLead, idFuncionarioDestino)`, `responderTransferencia(idLead, acao)`, `cancelarTransferencia(idLead)`
